...
## 2.6 dsh-taskqueue —— 客户端任务队列（低谷时段自动执行）

用户先把不着急的需求入队，队列在**官方低谷价时段**（动态解析官方页：当前周一至周五 9-12/14-18 高峰，其余低谷）自动派发给 agent 执行；也支持指定时间。派发前可查看/修改/取消/立即执行。

**交互（两种方式）**：
- 斜杠命令：`/queue help` 查看全部用法（add / list / cancel / edit / run-now / target / help，`--at` 支持 `HH:mm` 与 `YYYY-MM-DD HH:mm` 北京时区）；
- 模型工具（不依赖客户端命令系统）：`queue_add` / `queue_list` / `queue_cancel` / `queue_edit` / `queue_runnow`——自然语言即可调用。

**机制**：
- 持久化：`/data/dsh/taskqueue.json`（重启不丢）；
- 低谷判定：每次从官方定价页解析"高峰时段为北京时间…"定义，缓存 30 分钟，解析失败回退内置（周一至周五 9-12/14-18）；
- 派发：到点且 agent 空闲时，`agent.followup` 把 `[任务队列] <内容>` 投递进目标会话（默认：首次使用 /queue 的会话），agent 执行；
- 无在线会话时任务保持排队，下次检查重试。

**已知坑（重要）**：斜杠命令必须带 `input: { hint }` 描述符，否则客户端对**带参数**的命令（如 `/queue list`）不拦截、直接当普通文本发给模型——`/queue` 曾踩过此坑（已修复）。

**文件**：
### `dsh-taskqueue/lib/index.js`
```
import { readFile, writeFile } from "node:fs/promises";
...
### `dsh-taskqueue/package.json`
```
{
  "name": "@deepseek-ai/dsh-taskqueue",
  "version": "0.1.0",
  "description": "Client-level task queue: dump non-urgent ideas, auto-execute during off-peak (cheap) windows or at set times (local helper plugin, not an official package)",
  "type": "module",
  "main": "lib/index.js",
  "exports": {
    ".": "./lib/index.js",
...
✅ 新发现(2026-08-23实测): 本容器(dsh)的 docker 网桥网关 172.25.0.1 = NAS 宿主,
   ...
   用法: python3 /workspace/scripts/nas_ssh.py '远程命令' (SSH_ASKPASS自动喂密码,
